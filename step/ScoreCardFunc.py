...
def CalcWOE(df, col, target):
    ...
    regroup = regroup.replace(0, 1)                                         #把消费零笔变成消费1笔
    ...
    G = N - B
    regroup['bad_pcnt'] = regroup['bad'].map(lambda x: x * 1.0 / (1 if B == 0 else B))
    regroup['good_pcnt'] = regroup['good'].map(lambda x: x * 1.0 / (1 if G==0 else G))
    regroup['WOE'] = regroup.apply(lambda x: np.log((0.001 if x.good_pcnt==0 else x.good_pcnt) * 1.0 /(0.001 if x.bad_pcnt==0 else x.bad_pcnt)), axis=1)
    WOE_dict = regroup[[col, 'WOE']].set_index(col).to_dict(orient='index')
    for k, v in WOE_dict.items():
        WOE_dict[k] = v['WOE']
    IV = regroup.apply(lambda x: (x.good_pcnt - x.bad_pcnt) * np.log((0.001 if x.good_pcnt==0 else x.good_pcnt)* 1.0 / (0.001 if x.bad_pcnt==0 else x.bad_pcnt)), axis=1)
    IV = sum(IV)
...
def ChiMerge_MaxInterval(df, col, target, max_interval=10, special_attribute=[]):
    '''
    ...
    if N_distinct <= max_interval:  # If the raw column has attributes less than this parameter, the function will not work
        ...
        if len(special_attribute) >= 1:
            ...
            #isin函数返回T/F    #loc切片函数 提取对于变量col中不是-1的行
        else:
            df2 = df.copy()
        N_distinct = len(list(set(df2[col])))
        # Step 1: group the dataset by col and work out the total count & bad count in each level of the raw column
        if N_distinct > 50:
            ind_x = [int(i / 50.0 * N_distinct) for i in range(1, 50)]
            split_x = [colLevels[i] for i in ind_x]
            df2['temp'] = df2[col].map(lambda x: AssignGroup(x, split_x))    #关于变量col的取值数分组
        else:
            df2['temp'] = df[col]
        ...
        #进行卡方计算
        while (len(
                groupIntervals) > split_intervals):  # the termination condition: the number of intervals is equal to the pre-specified threshold
...
def Stats(df,col,keys):
    Tcnt = df.groupby(col)[keys].count()
    Tcnt = pd.DataFrame({'Total':Tcnt})
    Bcnt = df.groupby(col)[keys].sum()
    Bcnt = pd.DataFrame({"Bad":Bcnt})
    Summary = Tcnt.merge(Bcnt,left_index=True, right_index=True, how='left')
    Summary['Good'] = Summary['Total'] - Summary['Bad']
    return Summary

# def ChiMerge_MaxInterval2(df, col, target, max_interval=10, special_attribute=[]):
#     if len(special_attribute) >= 1:
#         df1 = df.loc[~df[col].isin(special_attribute)][[col,'target']]
#     else:
#         df1 = df[[col,'target']]
#     temp_bins,temp_cutoff = pd.qcut(df1[col],50,duplicates='drop',retbins= True)
#     if len(temp_bins) < max_interval:
#         print("The number of original levels for {} is less than or equal to max intervals".format(col))
#         return temp_cutoff[:-1]
#     else:
#         df1['temp'] = df1[col].map(lambda x:AssignBin(x,temp_cutoff[:-1]))
#         Summary = Stats(df1,'temp','target')
#         N = sum(Summary['Total'])
#         B = sum(Summary['Bad'])
#         overallRate = B * 1.0 / N
#         colLevels = list(Summary['temp'])
...
def BadRateMonotone(df, sortByVar, target, special_attribute=[]):
    ...
    :return:
    '''
    df2 = df.loc[~df[sortByVar].isin(['Bin '+str(i) for i in special_attribute])]
